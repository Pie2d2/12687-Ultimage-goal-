...
public class MaristBaseRobot2019_Quad_14101 {
    ...
    public DcMotor ringGrab = null;
    public Servo claw   = null;
    public Servo difPlate   = null;
    public Servo feed    = null;
    ...
    public MaristBaseRobot2019_Quad_14101 () {//potato

    }

    /* Initialize standard Hardware interfaces */
    public void init(HardwareMap ahwMap) {
        // Save reference to Hardware map//pota
        hwMap = ahwMap;
        ...
        ringGrab    =hwMap.dcMotor.get("ringgrab");
        ...
        claw = hwMap.servo.get("leftClaw");
        difPlate = hwMap.servo.get("rightClaw");
        feed = hwMap.servo.get("feed");
        ringServo = hwMap.servo.get("ringservo");
    ...
    public void liftMotorDegSet(double speed,
                             double deg,
                             double timeoutS) {
        int target;
    ...
    public void ringGrabDeg(double speed,
                             double deg,
                             double timeoutS) {
        int target;
        ...
        ringGrab.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);

        ringGrab.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        ...
        if (true) {       // Swapped out to include in MaristBaseRobot
            ...
            // Determine new target position, and pass to motor controller
            target = ringGrab.getCurrentPosition() + (int)(deg * 288);
            ringGrab.setTargetPosition(target);

            // Turn On RUN_TO_POSITION
            ringGrab.setMode(DcMotor.RunMode.RUN_TO_POSITION);

            // reset the timeout time and start motion.
            period.reset();
            ringGrab.setPower(Math.abs(speed));
    ...
    public void goToA(){
        strafeInches(.5, 30, 10);
        delay(1);
        driveStraightInches(.7, 40, 10);
        delay(1);
        pointTurnDegrees(.5, 180, 5);
        delay(1);
        liftMotorDegSet(0.7, 201, 5);
        delay(1);
        claw.setPosition(1);
        delay(5);
        claw.setPosition(.15);
        delay(1);
        liftMotorDegSet(0.7, 0, 5);
        delay(1);
    }
    public void goToC(){
        robot.strafeInches(.5, 30, 10);
        delay(1);
        robot.driveStraightInches(.7, 130, 10);
        delay(1);
        robot.pointTurnDegrees(.5, 180, 5);
        delay(1);
        robot.liftMotorDegSet(0.7, 201, 5);
        delay(1);
        ...
        public void ringMotorDegSet(double speed,
                             double deg,
                             double timeoutS) {
        int target = 0;
